...
func (s *Server) Start() error {
	...
	slog.Info("distrigo-kv server running", "listenAddr", s.ListenAddr)
...
func (s *Server) handleMessage(msg Message) error {
	...
	case SetCommand:
		if v.ttl > 0 {
			s.kv.SetWithTTL(v.key, v.val, time.Duration(v.ttl)*time.Second)
			} else {
				s.kv.Set(v.key, v.val)
			}
			resp.NewWriter(msg.peer.conn).WriteString("OK")
	case GetCommand:
...
func main() {
	listenAddr := flag.String("listenAddr", defaultListenAddr, "listen address of the distrigo-kv server")
	flag.Parse()
